   ...
   public function __construct()
    ...
    public function adScanner()
    {
        ...
        $profile = DB::table('staff')
                ...
                ->where(['id'=> $userid])
                ->first();
        return view('admin.admin-scanner', ['profile' => $profile]);
    }

    public function adScanDetail()
    {
        $userid = Auth::user()->id;
        $profile = DB::table('staff')
                ->select('staff.*')
    ...
    public function adScanAttend()
    {
        ...
        {
            $session = "1";
            DB::update('UPDATE studentxmodules SET status="ATTENDED", time=now(), remark="-", updated_at=now() WHERE studentid=? AND smsession=?', [$studId, $session]);
        }
        else if($v >= "14:00" && $v <= "17:00")
        {
            $session = "2";
        ...
        {
            DB::update('UPDATE studentxmodules SET status="NOT ATTENDED", time=now(), remark="-", updated_at=now() WHERE studentid=? AND status IS NULL', [$studId]);
